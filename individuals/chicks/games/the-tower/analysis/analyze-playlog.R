...
cat("\n=== RECENT DISSONANT RUN STATS TABLE ===\n\n")

# Dissonant runs live at regular (positive) tiers. Use positive_tiers_df
# (not df_all, which is post-efficiency-filter and drops Utility runs whose
# min/billion is huge due to tiny coin rewards).
dissonant_df <- positive_tiers_df %>%
  filter(!is.na(`Dissonant Run`) & `Dissonant Run` != "" & Tier > 0) %>%
  mutate(
    `Finish Wave` = as.integer(`Finish Wave`),
    `Max Wave` = as.integer(`Max Wave`)
  ) %>%
  filter(!is.na(`Finish Wave`) & `Finish Wave` > 0 & !is.na(`Max Wave`) & `Max Wave` > 0) %>%
  mutate(
    `Pct of Max` = `Finish Wave` / `Max Wave` * 100,
    Category = factor(`Dissonant Run`,
                      levels = c("Attack", "Defense", "Utility", "Ultimate Weapons"))
  )

if (nrow(dissonant_df) == 0) {
  cat("No dissonant runs found. Skipping dissonant stats table generation.\n")
} else {
  # Latest run per (Tier, Category)
  dissonant_latest <- dissonant_df %>%
    group_by(Tier, Category) %>%
    filter(Date == max(Date)) %>%
    arrange(Tier) %>%
    slice(1) %>%  # tie-break: first row at max date
    ungroup() %>%
    arrange(desc(Tier), Category)

  today <- Sys.Date()
  dissonant_latest <- dissonant_latest %>%
    mutate(`Days Since` = as.integer(today - Date))

  dissonant_table_rows <- vapply(seq_len(nrow(dissonant_latest)), function(i) {
    row <- dissonant_latest[i, ]
    sprintf(
      "| %d | %s | %s | %d | %d | %s%% | %d |",
      row$Tier, as.character(row$Category),
      format(row$Date, "%Y-%m-%d"),
      as.integer(row$`Finish Wave`), as.integer(row$`Max Wave`),
      fmt_dec(row$`Pct of Max`),
      row$`Days Since`
    )
  }, character(1))

  dissonant_all_count <- nrow(dissonant_df)
  dissonant_all_max_finish <- max(dissonant_df$`Finish Wave`, na.rm = TRUE)
  dissonant_avg_pct <- mean(dissonant_df$`Pct of Max`, na.rm = TRUE)
  dissonant_latest_date <- max(dissonant_df$Date)
  dissonant_days_since <- as.integer(today - dissonant_latest_date)

  dissonant_all_row <- sprintf(
    "| **All** | **%d runs** | **%s** | **%d** | \u2014 | **%s%%** | **%d** |",
    dissonant_all_count,
    format(dissonant_latest_date, "%Y-%m-%d"),
    dissonant_all_max_finish,
    fmt_dec(dissonant_avg_pct),
    dissonant_days_since
  )

  dissonant_stats_table <- c(
    sprintf("### Recent Dissonant Run Statistics (as of %s)", format(today, "%Y-%m-%d")),
    "",
    ...
    "| ----- | -------- | -------- | ----------: | -------: | ---------: | ---------: |",
    dissonant_table_rows,
    dissonant_all_row,
    ""
  )

  cat(paste(dissonant_stats_table, collapse = "\n"), "\n")

  dissonant_start_marker <- "<!-- RECENT-DISSONANT-STATS-START -->"
  dissonant_end_marker <- "<!-- RECENT-DISSONANT-STATS-END -->"

  readme_lines <- readLines(readme_path, warn = FALSE)

  d_start_idx <- which(readme_lines == dissonant_start_marker)
  d_end_idx <- which(readme_lines == dissonant_end_marker)

  if (length(d_start_idx) != 1 || length(d_end_idx) != 1) {
    stop("Expected exactly one RECENT-DISSONANT-STATS-START and one RECENT-DISSONANT-STATS-END marker in README.md. Found ", length(d_start_idx), " start and ", length(d_end_idx), " end markers.")
  }

  after_d_end <- if (d_end_idx < length(readme_lines)) readme_lines[(d_end_idx + 1):length(readme_lines)] else character(0)

  new_readme <- c(
    readme_lines[1:d_start_idx],
    "",
    "<!-- Auto-generated by analyze-playlog.R \u2014 do not edit between these markers -->",
    "",
    dissonant_stats_table,
    dissonant_end_marker,
    after_d_end
  )

  writeLines(new_readme, readme_path)
  cat("Updated README.md with recent dissonant run stats table.\n")
}
